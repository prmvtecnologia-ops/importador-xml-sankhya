...
<div class="layout">
    <aside>
        ...
        <div class="env">
            <span>Modo</span>
            <strong><?= $app['sandbox'] ? 'Simulação' : 'Produção' ?></strong>
        </div>
    </aside>
    ...
    <main>
        <header>
            <div>
                ...
                <p>Upload de XML, validação e geração de payload para TOP <?= htmlspecialchars((string)$api['codtipoper']) ?>.</p>
            </div>
            <span class="badge"><?= $app['sandbox'] ? 'Seguro para testes' : 'Envia para Sankhya' ?></span>
        </header>
        ...
        <section class="grid">
            ...
            <div class="card">
                ...
                <?php else: ?>
                    <div class="kpis">
                        <div><span>Parceiro</span><strong><?= htmlspecialchars($xmlData['cliente']['nome']) ?></strong></div>
                        <div><span>Documento</span><strong><?= htmlspecialchars($xmlData['cliente']['documento']) ?></strong></div>
                    ...
                    <form method="post" action="/process.php">
                        <input type="hidden" name="xml_file" value="<?= htmlspecialchars($fileName) ?>">
        ...
        <section class="card">
            <h2>Parceiro</h2>
            <div class="details">
                <div><span>Nome</span><strong><?= htmlspecialchars($xmlData['cliente']['nome']) ?></strong></div>
                <div><span>CPF/CNPJ</span><strong><?= htmlspecialchars($xmlData['cliente']['documento']) ?></strong></div>
                <div><span>Cidade</span><strong><?= htmlspecialchars($xmlData['cliente']['cidade']) ?></strong></div>
                <div><span>UF</span><strong><?= htmlspecialchars($xmlData['cliente']['uf']) ?></strong></div>
            </div>
        </section>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <?php foreach ($xmlData['itens'] as $item): ?>
                        <tr>
                            <td><?= htmlspecialchars($item['cprod']) ?></td>
                            <td><?= htmlspecialchars($item['cean'] ?: '-') ?></td>
